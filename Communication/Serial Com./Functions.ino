...
static byte dmp_updates[29][9] =
{
  ...
  ,
  {
    0x07, 0x41, 0x05, 0xF1, 0x20, 0x28, 0x30, 0x38 } //CFG_8 inv_write_quaternion
  ,
  {
    0x07, 0x7E, 0x01, 0x30 } //CFG_16 inv_set_footer
  ,
  {
    0x07, 0x46, 0x01, 0x9A } //CFG_GYRO_SOURCE inv_write_gyro
  ,
  {
    0x07, 0x47, 0x04, 0xF1, 0x28, 0x30, 0x38 } //CFG_9 inv_write_gyro -> inv_construct3_fifo
  ,
  {
    0x07, 0x6C, 0x04, 0xF1, 0x28, 0x30, 0x38 } //CFG_12 inv_write_accel -> inv_construct3_fifo
  ,
  {
...
void dmp_init(){
  ...
  for(int i = 0; i < 7; i++){
    ...
    for(byte j = 0; j < 16; j++){
      ...
      Wire.write(MEM_R_W);
      for(int k = 0; k < 16; k++){
        unsigned char byteTowrite = pgm_read_byte(&(dmpMem[i][j][k]));
        Wire.write((byte) byteTowrite);
      }
      Wire.endTransmission();
    }
  }
  ...
  for(byte j = 0; j < 8; j++){
    ...
    Wire.write(MEM_R_W);
    for(int k = 0; k < 16; k++){
      unsigned char byteTowrite = pgm_read_byte(&(dmpMem[7][j][k]));
      Wire.write((byte) byteTowrite);
    }
    Wire.endTransmission();
  }
  ...
  Wire.write(MEM_R_W);
  for(int k = 0; k < 9; k++){
      unsigned char byteTowrite = pgm_read_byte(&(dmpMem[7][8][k]));
      Wire.write((byte) byteTowrite);
  }
  Wire.endTransmission();
...
void processQuat(){
    ...
    processed_packet[2] = readd_packet[4];
    processed_packet[3] = readd_packet[5];
